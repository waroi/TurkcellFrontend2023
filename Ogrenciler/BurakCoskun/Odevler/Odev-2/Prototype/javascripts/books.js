function Books(name, author, year, genre, image) {
...
Books.prototype.generateUniqueId = function () {
  ...
  if (
    ...
    if (authorBooks.length === 1) {
      ...
      if (existingAuthors.has(author.toLowerCase())) {
        for (let i = 0; i < optionElements.length; i++) {
          if (optionElements[i].value.toLowerCase() === author.toLowerCase()) {
...
Books.prototype.clearAllBooks = function (e) {
  bookList.innerHTML = "";
  localStorage.removeItem("books");
  authorSelect.innerHTML = `<option value="All">Tüm Yazarlar</option>`;
};

Books.prototype.searchBooks = function (e) {
  const books = storage.getBooksFromStorage();
...
Books.prototype.filterBooks = function (e) {
  const books = storage.getBooksFromStorage();
  const selectedGenres = Array.from(
    document.querySelectorAll(".form-check-input:checked")
  );
...
Books.prototype.orderBooks = function (e) {
  const books = storage.getBooksFromStorage();
