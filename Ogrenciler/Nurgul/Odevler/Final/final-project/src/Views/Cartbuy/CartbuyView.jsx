...
import CartItem from "../../Components/Cartbuy/CartbuyItem";
...
const CartView = () => {
  ...
  const updateCart = async () => {
    ...
    await axios.put(`http://localhost:3000/carts/${user.id}`, {
      id: user.id,
      cart: updatedCart.filter((cartItem) => cartItem.demand > 0),
    });
  };
  ...
  return (
    <div>
      {cart.length > 0 ? (
        cart.map((el, i) => (
          <CartItem key={i} cartItem={el} setCart={setCart} />
        ))
      ) : (
        <h2>Cart is empty</h2>
      )}
      <CartBuy disabled={cart.length == 0} toBuy={toBuy} />
    </div>
  );
};
